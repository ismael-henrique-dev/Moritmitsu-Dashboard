...
import { FetchClassDetailsResponse } from '@/lib/definitions'
import { cookies } from 'next/headers'

export async function fetchClasseDetailsById(id: string) {
  try {
    const cookieStore = await cookies()
    const accessToken = cookieStore.get('accessToken')?.value

    const { data: response } = await api.get<FetchClassDetailsResponse>(
      `/classes/details/${id}`,
      {
        headers: {
